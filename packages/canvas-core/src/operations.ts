...
function applyOperation(
  ...
  switch (op.type) {
    case "node.upsert":
      return {
        ...doc,
        nodes: doc.nodes.some((n) => n.id === op.node.id)
          ? doc.nodes.map((n) => (n.id === op.node.id ? op.node : n))
          : [...doc.nodes, op.node],
      };
    case "node.remove": {
    ...
    case "node.resize":
      return {
        ...doc,
        nodes: doc.nodes.map((n) =>
          n.id === op.nodeId ? { ...n, ...op.size } : n,
        ),
      };
    case "connection.upsert":
      return {
        ...doc,
        connections: doc.connections.some((e) => e.id === op.connection.id)
          ? doc.connections.map((e) =>
              e.id === op.connection.id ? op.connection : e,
            )
          : [...doc.connections, op.connection],
      };
    case "connection.remove": {
      const ids = new Set(op.connectionIds);
      return {
        ...doc,
        connections: doc.connections.filter((e) => !ids.has(e.id)),
      };
    }
    case "viewport.set":
