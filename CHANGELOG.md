...
+ [安全] Canvas Agent 启动日志不再打印本地 Connect token，避免终端与采集日志泄露凭据。
+ [新增] 本地 Agent 支持检索 GitHub Skill 目录并从结果进入 commit 固定、文件清单与权限确认安装流程。
...
+ [新增] Model Gateway 接收 Worker 运行态健康反馈，连续三次失败自动进入 60 秒 cooldown 并从路由候选排除，成功调用恢复 healthy。
