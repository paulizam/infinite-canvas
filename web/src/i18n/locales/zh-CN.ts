...
export default {
    ...
    canvas: {
        ...
        workflow: { button: "Workflow", title: "发布与执行 Workflow", unpublished: "尚未发布", canvasRevision: "当前 Canvas revision：{{revision}}", refresh: "刷新", publish: "发布新版本", diagnostics: "编译诊断", locate: "定位节点", history: "版本历史", noVersions: "暂无已发布版本", loadFailed: "加载 Workflow 失败", publishFailed: "发布 Workflow 失败", execution: { title: "执行中心", selectNodes: "选择起始节点", runAll: "全量执行", runSelected: "从选中节点执行", cancel: "取消执行", empty: "暂无执行记录", timeline: "执行时间线", snapshots: "输入/输出快照", loadFailed: "加载执行记录失败", runFailed: "创建执行失败", cancelFailed: "取消执行失败", retryFailed: "重试节点失败" }, api: { title: "公开 Workflow API", loadFailed: "加载 API token 失败", createFailed: "创建 API token 失败", rotateFailed: "轮换 API token 失败", revokeFailed: "吊销 API token 失败", name: "Token 名称", create: "创建 Token", rotate: "轮换 Token", revoke: "吊销 Token", secretTitle: "保存 API Secret", secretOnce: "Secret 仅显示一次，请立即安全保存。", copySnippet: "复制 API snippet" }, library: { title: "Workflow 资源库", loadFailed: "加载资源库失败", folderFailed: "文件夹操作失败", updateFailed: "更新 Workflow 失败", exportFailed: "导出 Workflow 失败", importFailed: "导入 Workflow 失败", tooLarge: "导入文件不能超过 2 MiB", folderName: "新文件夹", import: "导入", noDescription: "暂无描述", folder: "文件夹", edit: "编辑 Workflow 信息", description: "描述", tags: "标签", coverAsset: "封面 Asset ID" } },
        ...
        configNode: { title: "生成配置", image: "生图", text: "文本", video: "视频", audio: "音频", prompt: "提示词", references: "参考图", videoReferences: "参考视频", audioReferences: "参考音频", items: "{{count}} 个", images: "{{count}} 张", compose: "组装提示词", stop: "停止", generate: "开始生成", billingLoading: "正在预估积分…", billingUnavailable: "积分预估暂不可用", billingEstimate: "预计 {{points}} 积分", billingBalance: "余额 {{points}}", insufficientPoints: "积分不足" },
        projectPage: {
        ...
        superResolution: { title: "AI 超分", description: "使用当前图片模型重建高清细节，并生成与原图相连的新节点。", model: "生成模型", defaultModel: "默认图片模型", detail: "细节策略", faithful: "忠于原图", enhanced: "增强细节", submit: "开始 AI 超分", resultTitle: "AI 超分结果", faithfulPrompt: "对参考图片进行 AI 超分辨率重建，目标长边 {{target}} 像素。严格保持原始主体、构图、比例、颜色、文字和风格，不新增或删除内容；仅消除压缩伪影、锯齿与噪点，恢复自然清晰的纹理。只返回处理后的图片。", enhancedPrompt: "对参考图片进行 AI 超分辨率重建，目标长边 {{target}} 像素。保持原始主体、构图、比例、颜色、文字和风格，在不改变语义的前提下增强可信的边缘、材质与微小纹理；禁止新增对象。只返回处理后的图片。" },
        editors: {
    ...
    agent: {
        cloud: { loadFailed: "加载云 Agent 失败", session: "选择会话", newSession: "新会话名称", create: "创建", prompt: "描述需要 Agent 完成的创作任务", run: "开始任务", detail: "任务详情", model: "逻辑模型", skills: "允许的 Skills", applyCanvas: "应用到当前 Canvas", canvasApplied: "已应用到 Canvas", openAssets: "在素材库查看", sendDrama: "发送至短剧", sentDrama: "已发送至短剧", dramaId: "短剧项目 ID", dramaRevision: "当前短剧 revision", dramaEntityKind: "实体类型", dramaEntityName: "实体名称", character: "角色", scene: "场景", prop: "道具", description: "描述", entityPrompt: "实体 Prompt", sortOrder: "排序", decline: "拒绝", approve: "批准" },
        ...
        skillManager: {
            codexBusy: "Codex 正在运行，请完成当前任务后再提炼 Skill", noConversation: "当前对话还没有可提炼的已完成内容", noCanvas: "当前页面没有可提炼的画布", connecting: "当前页面仍在连接 Agent，请稍后再试", syncFailed: "同步当前画布失败，请检查 Agent 连接后重试", noDraft: "未生成 Skill 草稿", draftCreated: "草稿已生成，可在技能页确认后创建", draftFailed: "生成 Skill 草稿失败", contentMissing: "未读取到 Skill 内容", readFailed: "读取 Skill 失败", defaultPromptMention: "默认提示词需要包含 ${{name}}", created: "Skill 已创建", updated: "Skill 已更新", saveFailed: "保存 Skill 失败", deleteTitle: "删除 {{name}}", deleteDescription: "删除后本地文件无法恢复，确定继续吗？", delete: "删除", deleted: "Skill 已删除", deleteFailed: "删除 Skill 失败", statusFailed: "更新 Skill 状态失败", installGithub: "从 GitHub 安装", installGithubDescription: "先检查来源、文件与权限，再写入本机", searchGithub: "搜索 GitHub Skills", searchPlaceholder: "输入用途或关键词", searchAction: "搜索", searchRequired: "搜索词至少需要 2 个字符", searchFailed: "GitHub Skill 搜索失败", installSource: "GitHub Skill 地址", installSourceRequired: "请输入 GitHub Skill 地址", previewInstall: "安全预览", installPreviewMissing: "未读取到安装预览", installPreviewFailed: "生成安装预览失败", installNow: "确认并安装", installFiles: "来源文件（{{count}}）", installPermissions: "安装权限确认", installNoPermissions: "未声明或推断出额外权限", installSafetyNote: "安装内容已固定到 commit SHA；安装器不会执行远程脚本。", installed: "{{name}} 已安装", installFailed: "安装 Skill 失败",
        ...
        panel: { connectionSettings: "连接设置 · {{status}}", connectionSettingsLabel: "连接设置，当前{{status}}", chat: "对话", history: "历史", skills: "技能", runs: "云任务", logs: "日志", collapse: "收起对话", collapseLabel: "收起 Agent 面板", resize: "调整右侧面板宽度", content: "Agent 内容", mcpInitializing: "MCP 初始化中，完成后即可发送", initFailed: "Codex 对话初始化失败，请新建或恢复对话", placeholder: "询问 Codex，输入 / 使用技能，@ 引用画布素材" },
    ...
    studio: {
        studioView: "创意工作室", canvasView: "画布视图", projects: "返回项目", revision: "项目版本 r{{revision}}", refresh: "刷新云端成果", search: "搜索节点、Prompt 或内容", allNodes: "全部节点", resultCount: "{{count}} 项成果", sameProjectHint: "双视图共享同一项目数据", empty: "当前筛选下暂无成果", projectMissing: "项目不存在或尚未同步", loadFailed: "云端成果加载失败", downloadFailed: "下载失败", download: "下载成果", focusNode: "回到画布定位节点",
        kind: { all: "全部媒体", text: "文本", image: "图片", video: "视频", audio: "音频" },
