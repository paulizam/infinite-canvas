...
class AdminPlatform {
    ...
    volcengine = (id: string, kind: "models" | "resources" | "usage") =>
        this.get<{ kind: string; payload: unknown; resourceUsage?: Array<{ configurationCode: string; quota: number; used: number; remaining: number; expired: number; unit: string }> }>(`/models/channels/${id}/volcengine/${kind}`);
