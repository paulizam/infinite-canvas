import { Check, Download, Folder, Pencil, Star, Trash2, X } from "lucide-react";
...
import { inferCanvasProjectCover } from "@/lib/canvas/canvas-project-organization";
...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const updateProjectOrganization = useCanvasStore((state) => state.updateProjectOrganization);
    ...
    const cover = inferCanvasProjectCover(project);
    const open = () => {
    ...
    return (
        <article className="group relative flex min-h-44 cursor-pointer flex-col justify-between overflow-hidden rounded-2xl bg-[#f1eee8] p-5 transition hover:bg-[#ebe6dc] dark:bg-white/5 dark:hover:bg-white/10" onClick={() => !editing && open()}>
            {cover ? <img src={cover} alt="" className="absolute inset-0 h-full w-full object-cover opacity-10" /> : null}
            <div className="relative flex flex-1 flex-col justify-between">
                        ...
                        {editing ? (
                            <>
                                <Button
                                    ...
                                    size="small"
                                    shape="circle"
                                    icon={<Star className={`size-4 ${project.favorite ? "fill-amber-400 text-amber-500" : ""}`} />}
                                    onClick={() => updateProjectOrganization(project.id, { favorite: !project.favorite })}
                                    aria-label={t("canvas.project.favorite")}
                                />
                ...
                <div className="relative mt-3 flex items-center gap-2" onClick={(event) => event.stopPropagation()}>
                    <Folder className="size-3.5 text-stone-500" />
                    <Input size="small" variant="borderless" value={project.folderId || ""} placeholder={t("canvas.project.rootFolder")} onChange={(event) => updateProjectOrganization(project.id, { folderId: event.target.value.trim() || null })} />
                </div>
            </div>
        </article>
    );
