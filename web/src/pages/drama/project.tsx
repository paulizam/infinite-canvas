...
type Action = "project" | "script" | "entity" | "shot" | "generation" | "selection" | "timeline" | "review" | "render" | "interop" | null;
...
export default function DramaProjectPage() {
    ...
    const load = useCallback(async () => {
        ...
        setError("");
        try {
            const [nextDetail, nextProduction, nextRenders] = await Promise.all([
    ...
    const items = [
        { key: "script", label: `剧本 (${detail.scripts.length})`, children: <ScriptPanel detail={detail} onAdd={() => open("script")} /> },
    ...
    return (
        <main className="h-full overflow-y-auto bg-stone-50 dark:bg-stone-950">
            ...
            <Modal title={actionTitle(action)} open={!!action} okText="提交" cancelText="取消" confirmLoading={saving} onOk={() => void submit()} onCancel={() => setAction(null)} destroyOnHidden width={680}>
                <Form form={form} layout="vertical" className="pt-3">{actionFields(action, shotOptions, generationOptions)}</Form>
            </Modal>
        </main>
    );
}

function ScriptPanel({ detail, onAdd }: { detail: CloudDramaDetail; onAdd: () => void }) { return <><Button type="primary" icon={<Plus className="size-4" />} onClick={onAdd}>新建剧本版本</Button><List className="mt-4" dataSource={detail.scripts} locale={{ emptyText: "暂无剧本版本" }} renderItem={(item) => <List.Item><List.Item.Meta title={<Space>v{item.version}<Tag>{item.operation}</Tag><Tag color={item.reviewStatus === "approved" ? "green" : "default"}>{item.reviewStatus}</Tag></Space>} description={<Typography.Paragraph ellipsis={{ rows: 3, expandable: true }}>{item.content}</Typography.Paragraph>} /></List.Item>} /></>; }
...
function actionFields(action: Action, shots: Array<{ value: string; label: string }>, generations: Array<{ value: string; label: string }>) {
...
function actionTitle(action: Action) { return ({ project: "项目设置", script: "新建剧本版本", entity: "添加实体档案", shot: "添加镜头", generation: "生成镜头媒体", selection: "选择生成版本", timeline: "添加时间轴项目", review: "镜头审批与批注", render: "创建交付任务", interop: "Canvas / 素材互通" } as Record<string, string>)[action || ""] || ""; }
