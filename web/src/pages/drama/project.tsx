...
function actionFields(action: Action, shots: Array<{ value: string; label: string }>, generations: Array<{ value: string; label: string }>, scripts: Array<{ value: string; label: string }>) {
    ...
    if (action === "render") return <><Alert className="mb-4" type="info" showIcon message={tr("renderHint")} /><Form.Item name="kind" label={tr("deliveryType")} rules={required}><Select options={[{ value: "ffmpeg", label: tr("ffmpegMovie") }, { value: "jianying", label: tr("jianyingZip") }]} /></Form.Item><Form.Item name="settings" label={tr("renderSettings")} rules={required}><Input.TextArea rows={6} /></Form.Item></>;
